...
% kinetic rate constant for unbinding, k_off. Input different values for koff_Values and
% the model will run 10 (or another amount) iterations of the simulation for each k_off value and
...
koff_Prep = ones(1,AverageIterations);  %sorts all values for cooperativity from smallest to largest
kineticRateConstants = [];
for f = 1:length(koff_Values)
    kineticRateConstants = sort([kineticRateConstants,koff_Values(f)*koff_Prep]);
end
...
%     subplot(2,1,1);
    hold on;
    scatter(t(Loops,:),FracCover(Loops,:),1,Colors(koff_Values == k_off,:),'filled','HandleVisibility','off');
...
%     subplot(2,1,2);
%     hold on;
%     scatter(t(Loops,:),DNA_Length(Loops,:)/1000,1,Colors(koff_Values == w,:),'filled','HandleVisibility','off');
%     ylabel('Length (\mum)');
    xlabel('Time, t');
%     xlim([0 1.25]);
...
for b = kineticRateConstants
    ...
    DNA_LengthMatrix(1:length(LoopNumbers),:) = DNA_Length(LoopNumbers,:);  %length profiles for the given ratio
...
for c = 1:length(kineticRateConstants)
    Legend{c} = ['k_o_f_f = ', num2str(kineticRateConstants(c))];
end
figure(2);
% subplot(2,1,1);
hleg = legend(Legend,'location','southeast');
htitle = get(hleg,'Title');
set(htitle,'String','Kinetic Rate Constant for Unbinding, k_o_f_f');